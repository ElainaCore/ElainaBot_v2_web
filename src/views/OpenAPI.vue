...
const loginStatusText = computed(() => loginStatus.value === 'logged_in' ? '登录成功！' : loginStatus.value === 'waiting' ? '等待扫码...' : '获取中...')
const loginStatusClass = computed(() => ({ 'status-ok': loginStatus.value === 'logged_in', 'status-waiting': loginStatus.value === 'waiting' }))
...
async function startLogin() {
  ...
  try {
    ...
    if (data.success) { loginUrl.value = data.login_url || ''; qrCode.value = data.qr_code || ''; qrImgUrl.value = data.login_url ? `https://api.2dcode.biz/v1/create-qr-code?data=${encodeURIComponent(data.login_url)}` : ''; loginStatus.value = 'waiting'; showLoginQR.value = true; pollLogin() }
    else alert(data.message || '获取二维码失败')
...
function pollLogin() {
  ...
  loginTimer = setInterval(async () => {
    try { const { data } = await axios.post('/api/openapi/check-login', { user_id: 'web_user', qr_code: qrCode.value }); if (data.success && data.status === 'logged_in') { loginStatus.value = 'logged_in'; loggedIn.value = true; loginInfo.uin = data.data?.uin || ''; loginInfo.appId = data.data?.appId || ''; stopLoginPoll(); setTimeout(() => { showLoginQR.value = false }, 800); fetchBots() } } catch {}
  }, 2000)
}
...
<template>
  <div class="openapi-page">
    ...
    <!-- Login QR modal -->
    <div v-if="showLoginQR" class="modal-overlay" @click.self="showLoginQR = false">
      <div class="qr-modal">
        <div class="qr-title">扫码登录 QQ 开放平台</div>
        <div class="qr-desc">请使用 QQ 扫描下方二维码</div>
        <div class="qr-frame">
          <img v-if="qrImgUrl" :src="qrImgUrl" class="qr-img" alt="登录二维码" />
          <div v-else class="qr-loading">正在生成二维码...</div>
        </div>
        <div :class="['qr-status', loginStatusClass]">{{ loginStatusText }}</div>
        <a v-if="loginUrl" :href="loginUrl" target="_blank" class="btn btn-ghost qr-link">或点击打开登录页面</a>
        <button class="btn btn-ghost qr-close" @click="showLoginQR = false">关闭</button>
      </div>
    </div>
